...
# ── Sales Agent Admin Endpoints ───────────────────────────────────────────────
from services.sales_agent.orchestrator import get_orchestrator

@app.post("/admin/sales-agent/run")
async def run_sales_agent(
    target_count: int = 5,
    current_user: dict = Depends(ceo_dashboard.require_admin),
):
    """
    Run sales agent to research and add new leads.
    Called by cron job daily.
    """
    try:
        orch = get_orchestrator()
        
        # Target companies (expand this list)
        targets = [
...
@app.get("/admin/sales-agent/status")
async def sales_agent_status(
    current_user: dict = Depends(ceo_dashboard.require_admin),
):
    """Get sales agent status and configuration."""
    return {
        "status": "active",
