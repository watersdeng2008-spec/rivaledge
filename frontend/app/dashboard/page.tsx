  ...
  const handleGeoCheckout = async () => {
    ...
    try {
      ...
      if (e instanceof ApiError) {
        ...
        setError('Failed to start GEO checkout');
      }
    } finally {
      setUpgradingPlan(false);
    }
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          {loadingBilling ? (
            ...
            <div className="flex items-center gap-6">
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="font-semibold capitalize">
                    {billing.plan === 'geo_selfservice' ? 'Done-For-You' : billing.plan}
                  </span>
                  {(billing.plan === 'pro' || billing.plan === 'geo_selfservice') && <Crown className="w-4 h-4 text-yellow-400" />}
                  {billing.plan === 'geo_selfservice' && (
                    <span className="inline-flex items-center gap-1 bg-purple-600/20 text-purple-400 text-xs px-2 py-0.5 rounded-full font-medium">
              ...
              {(billing.plan === 'pro' || billing.plan === 'geo_selfservice') && (
                <div>
                  <span className="text-xs text-slate-500 uppercase tracking-wider">AI Visibility</span>
                  <div className="flex items-center gap-2 mt-0.5">
                    {billing.has_geo_addon || billing.plan === 'geo_selfservice' ? (
                      <span className="inline-flex items-center gap-1 bg-purple-600/20 text-purple-400 text-xs px-2 py-0.5 rounded-full font-medium">
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" />
                        </svg>
          ...
          {(billing?.plan === 'solo' || billing?.plan === 'pro') && (
            <button
              onClick={handleGeoCheckout}
              disabled={upgradingPlan}
              className="flex items-center gap-2 bg-purple-600/10 hover:bg-purple-600/20 border border-purple-500/30 text-purple-400 px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
            >
          ...
          <div className="space-y-4">
            ...
            {billing?.plan === 'pro' && !billing?.has_geo_addon && (
              <button
                onClick={handleGeoCheckout}
                disabled={upgradingPlan}
                className="w-full flex items-center gap-3 bg-purple-600/10 hover:bg-purple-600/20 border border-purple-500/30 text-white px-5 py-4 rounded-xl text-sm font-medium transition-colors disabled:opacity-50 text-left"
              >
                <svg className="w-5 h-5 text-purple-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" />
                </svg>
                <div>
                  <div className="font-medium">Add AI Visibility</div>
                  <div className="text-slate-500 text-xs mt-0.5">Get cited by ChatGPT, Claude &amp; more</div>
                </div>
              </button>
            )}
            
            {billing?.has_geo_addon && (
              <div className="bg-purple-600/10 border border-purple-500/30 rounded-xl px-5 py-4">
                <div className="flex items-center gap-2 mb-2">
                  <svg className="w-4 h-4 text-purple-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9.813 15.904L9 18.75l-.813-2.846a4.5 4.5 0 00-3.09-3.09L2.25 12l2.846-.813a4.5 4.5 0 003.09-3.09L9 5.25l.813 2.846a4.5 4.5 0 003.09 3.09L15.75 12l-2.846.813a4.5 4.5 0 00-3.09 3.09zM18.259 8.715L18 9.75l-.259-1.035a3.375 3.375 0 00-2.455-2.456L14.25 6l1.036-.259a3.375 3.375 0 002.455-2.456L18 2.25l.259 1.035a3.375 3.375 0 002.455 2.456L21.75 6l-1.036.259a3.375 3.375 0 00-2.455 2.456zM16.894 20.567L16.5 21.75l-.394-1.183a2.25 2.25 0 00-1.423-1.423L13.5 18.75l1.183-.394a2.25 2.25 0 001.423-1.423l.394-1.183.394 1.183a2.25 2.25 0 001.423 1.423l1.183.394-1.183.394a2.25 2.25 0 00-1.423 1.423z" />
                  </svg>
                  <span className="font-medium text-sm text-purple-400">GEO Active</span>
                </div>
                <p className="text-slate-400 text-xs">Your company is optimized for AI search visibility. Monthly citation reports in progress.</p>
              </div>
            )}
